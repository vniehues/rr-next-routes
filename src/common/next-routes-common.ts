 ...
 * @param indexCreator - Function to create an index route entry
...
export function createRouteConfig(
  ...
  indexCreator: (file: string) => RouteConfigEntry
): RouteConfigEntry {
  // Handle index routes in dynamic folders
  if (
    routeFileNames.includes(name) &&
    ...
    const routePath =
      parentPath === ""
        ? routeName
        : `${parentPath.replace(folderName, "")}${routeName}`;
    return routeCreator(transformRoutePath(routePath), relativePath);
  }

  // Handle regular index routes - FIXED: Use indexCreator instead of routeCreator
  ...
  const routePath =
    parentPath === "" ? `/${routeName}` : `${parentPath}/${routeName}`;
  return routeCreator(transformRoutePath(routePath), relativePath);
}
  ...
  function scanDir(dirPath: string) {
    return {
      ...
      files: readdirSync(dirPath).sort((a, b) => {
        ...
        return routeFileNames.includes(aName) && extensions.includes(aExt)
          ? -1
          : 1;
      }),
    };
  ...
  function scanDirectory(
    ...
    files.forEach((item) => {
      ...
      if (stats.isDirectory()) {
        ...
        (layoutFile && !(enableHoistedFolders && isHoistedFolder(name))
          ? currentLevelRoutes
          : routes
        ).push(...nestedRoutes);
    ...
    if (layoutFile) {
      const layoutPath = join(
        baseFolder,
        relative(pagesDir, join(dir, layoutFile))
      );
